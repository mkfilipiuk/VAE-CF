...
from vae.config import BIN_DATA_DIR
from vae.load.parsers import save_as_npz
...
def load_data(path,
              val_ratio=0.1,
              test_ratio=0.1,
              val_movies_ratio=0.8,
              test_movies_ratio=0.8,
              seed=98765,
              only_first_users=None,
              **kwargs):
    """
    Downloads the data if not already downloaded. Then loades the data, filters, and splits into 5 matrices.
    :param path: Path to npz file created by parser
    :param val_ratio: Ratio of validation users to all users
    :param test_ratio: Ratio of test users to all users
    :param val_movies_ratio: Ratio of interactions in valid_in to valid_out
    :param test_movies_ratio: Ratio of interactions in test_in to test_out
    ...
    if only_first_users:
        data = data[:only_first_users, ]

    data = filter_data(data, **kwargs)

    train_matrix, val_matrix, test_matrix = split_into_train_val_test(data, val_ratio, test_ratio)

    train_matrix, val_matrix, test_matrix = \
        remove_items_that_doesnt_occure_in_train(train_matrix, val_matrix, test_matrix)

    val_train, val_test = split_movies_into_train_test(val_matrix, val_movies_ratio)
    test_train, test_test = split_movies_into_train_test(test_matrix, test_movies_ratio)

    return train_matrix, val_train, val_test, test_train, test_test



def load_and_parse_ML_20M(path, threshold=4, seed=98765, **kwargs):
    """
	...
	Modifications copyright (C) 2013 Michał Filipiuk, Albert Cieślak, Frederic Grabowski, Radosław Rowicki
    """

    CACHE_DIR = os.path.join(BIN_DATA_DIR, "ml_20m")
    ...
    source_file = os.path.join(path, "ratings.csv")
